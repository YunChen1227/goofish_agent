...
from goofish_agent.platform.auth import AuthManager
from goofish_agent.platform.browser import BrowserEngine
from goofish_agent.platform.parsers.chat_parser import ChatMessage, ChatParser
from goofish_agent.platform.parsers.detail_parser import DetailParser, ProductDetail
from goofish_agent.platform.parsers.search_parser import ProductBrief, SearchParser
...
class GoofishClient:
    ...
    async def search(self, query: str, filters: dict | None = None) -> list[ProductBrief]:
        ...
        await page.goto(f"https://www.goofish.com/search?q={query}")
        await self._anti.random_browse_pause()
        # TODO: apply filters (price range, location) via page interaction
    ...
    async def get_product_detail(self, product_id: str) -> ProductDetail | None:
        ...
        await page.goto(f"https://www.goofish.com/item/{product_id}")
        await self._anti.random_browse_pause()
